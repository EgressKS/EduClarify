...
//         const res = await apiClient.get("/commons-search", {
//           params: { query },
...
export default function DiagramPanel({ query, className = "", onClose }: { query?: string; className?: string; onClose?: () => void }) {
  const [items, setItems] = useState<Diagram[]>([])
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [selected, setSelected] = useState<Diagram | null>(null)
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        console.error("Error fetching diagrams:", e)
        const message = axios.isAxiosError(e)
          ? e.response?.data?.message || e.response?.data?.error || e.message || "Failed to load diagrams"
          : e instanceof Error
            ? e.message
            : "Failed to load diagrams"
        setError(message)
  ...
  return (
    ...
    >
      <div className="p-6 flex-shrink-0">
        ...
        {!loading && !error && items.length > 0 && (
          <div
            className={`grid grid-cols-1 sm:grid-cols-2 gap-6 flex-1 overflow-y-auto transition-all duration-700 delay-300 ${isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
          >
            {items.map((img, i) => (
              <div
                key={i}
